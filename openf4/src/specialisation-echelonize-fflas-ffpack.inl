...
#ifndef OPENF4_SPECIALISATION_ECHELONIZE_FFLAS_FFPACK_INL
#define OPENF4_SPECIALISATION_ECHELONIZE_FFLAS_FFPACK_INL
...
namespace F4
{
    ...
    Matrix<Element>::echelonizeRight (chrono::duration<int,milli> & tmp_ech_db, chrono::duration<int,milli> & tmp_ech_dh)
    {
        ...
        if(VERBOSE>3)
        {
            cout << "Time conversion 1 (right slice of the matrix into a fflas-ffpack matrix): " << chrono::duration_cast<millisecs_t>(chrono::steady_clock::now()-startc).count() << " ms" << endl;
        }
        ...
        if(VERBOSE>3)
        {
            cout << "Time conversion 2 (fflas-ffpack matrix into the right slice of the matrix): " << chrono::duration_cast<millisecs_t>(chrono::steady_clock::now()-startc).count() << " ms" << endl;
        }
        delete[] P;
        delete[] Q;
...
#endif // OPENF4_SPECIALISATION_ECHELONIZE_FFLAS_FFPACK_INL
