...
namespace F4
{
    ...
    Node::Node():_key(-1), _lt(false), _bf(0), _parent(0), _left(0), _right(0)
    {
    } 
    
    AvlMonomial::AvlMonomial(): _array(1000, 1000, 10), _it(_array.getBegin()), _root(0)
    {
    }
    ...
    {
        Node *tmpnode, *tmpnode2, *tmpnode3, *tmpnode4, *tmpnode5, *tmp_parent, *node;
        int adjust, cmp;
        ...
        {
            tmpnode2 = tmpnode;
            if ((tmpnode->_key) < numMon)
            {
                cmp = 1;
                tmpnode = tmpnode->_right;
            }
            else if ((tmpnode->_key) > numMon)
            {
                cmp = -1;
    ...
    Node *
    AvlMonomial::findNextBiggest(Node * node)
    {
        if (node == 0)
        {
            return 0;
